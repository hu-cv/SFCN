...
if (~isfield(para.param,'displayProgress'))
    para.param.displayProgress = 0;
end
totalerr(1) = 99999;
if (isfield(para.param,'errorFlag')==0)
    para.param.errorFlag = 0;
end

if (isfield(para.param,'TrueDictionary'))
    displayErrorWithTrueDictionary = 1;
    ErrorBetweenDictionaries = zeros(para.param.numIteration+1,1);
    ratio = zeros(para.param.numIteration+1,1);
else
    displayErrorWithTrueDictionary = 0;
	ratio = 0;
end
if (para.param.preserveDCAtom>0)
    FixedDictionaryElement(:,1) = 1/sqrt(size(Data,1));
else
    FixedDictionaryElement = [];
end

if (size(Data,2) < para.param.K)
    disp('Size of data is smaller than the dictionary size. Trivial solution...');
    Dictionary = Data(:,1:size(Data,2));
    return;
elseif (strcmp(para.param.InitializationMethod,'DataElements'))
    Dictionary(:,1:para.param.K) = Data(:,1:para.param.K-para.param.preserveDCAtom);
elseif (strcmp(para.param.InitializationMethod,'GivenMatrix'))
    Dictionary = para.param.initialDictionary(:,1:para.param.K-para.param.preserveDCAtom);
end
% reduce the components in Dictionary that are spanned by the fixed
% elements
if (para.param.preserveDCAtom)
    tmpMat = FixedDictionaryElement \ Dictionary;
    Dictionary = Dictionary - FixedDictionaryElement*tmpMat;
end
%normalize the dictionary.
Dictionary = Dictionary*diag(1./sqrt(sum(Dictionary.*Dictionary)));
totalErr = zeros(1,para.param.numIteration);

CoefMatrix = sparse(para.param.K,size(Data,2));

% the K-SVD algorithm starts here.
for iterNum = 1:para.param.numIteration
    % find the coefficients
	CoefMatrix = NN_BP(Data, [FixedDictionaryElement,Dictionary],para.param.L,CoefMatrix);
    ...
    if (iterNum>1 & para.param.displayProgress)
        if (para.param.errorFlag==0)
    ...
    if (displayErrorWithTrueDictionary ) 
        [ratio(iterNum+1),ErrorBetweenDictionaries(iterNum+1)] = I_findDistanseBetweenDictionaries(para.param.TrueDictionary,Dictionary);
    ...
    if (isfield(para.param,'waitBarHandle'))
        waitbar(iterNum/para.param.counterForWaitBar);
    end
end
